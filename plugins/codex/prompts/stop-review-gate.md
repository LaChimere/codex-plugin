<task>
Run a stop-gate review of the previous host-agent turn.
Only review the work from the previous host-agent turn.
Only review it if the host agent actually did code changes in that turn.
...
If the previous host-agent turn was only a status update, a summary, a setup/login check, a review result, or output from a command that did not itself make direct edits in that turn, return ALLOW immediately and do no further work.
...
Do not treat the previous host-agent response as proof that code changes happened; verify that from the repository state before you block.
